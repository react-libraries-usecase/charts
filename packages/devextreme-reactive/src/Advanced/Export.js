...
import { gaming as data } from './../data-virtualization';
...
const addKeyframe = (name, def) => {
  ...
  let style = Array.from(head.getElementsByTagName('style')).find(
    node => node.dataset[ANIMATIONS]
  );
  if (!style) {
...
const getLabelAnimationName = () => {
  ...
  addKeyframe(
    name,
    '{ 0% { opacity: 0; } 99% { opacity: 0; } 100% { opacity: 1; } }'
  );
  return name;
};
    ...
    return (
      <Paper id={rootContainerId}>
        <Chart data={chartData}>
          <Title
            text={
              'Estimated global gaming software\n(revenue by platform, in 2018)'
            }
