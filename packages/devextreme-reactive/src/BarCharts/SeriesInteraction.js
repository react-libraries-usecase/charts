...
import { annualVehiclesSales as data } from './../data-virtualization';
...
const TitleText = withStyles({ title: { marginBottom: '30px' } })(
  ({ classes, ...restProps }) => (
...
const AuxiliaryButton = props => <Button variant="outlined" {...props} />;

const AuxiliarySelection = ({ classes, target, turnNext, turnPrev, clear }) => (
  <div>
...
const AuxiliaryHover = ({ classes, target, enabled, toggle }) => (
...
const encodeTarget = ({ series, point }) =>
  2 * point + Number(series === 'China');
const decodeTarget = code => ({
  series: code % 2 ? 'China' : 'USA',
  point: Math.floor(code / 2)
});
    ...
    this.click = ({ targets }) => {
      ...
      if (target) {
        this.setState(({ selection }) => ({
          selection:
            selection[0] && compareTargets(selection[0], target) ? [] : [target]
        }));
      }
    };
    ...
    this.turnNextSelection = () =>
      this.setState(({ selection }) => {
        ...
        const newTarget = decodeTarget(
          Math.min(encodeTarget(target) + 1, 2 * data.length - 1)
        );
        return { selection: [newTarget] };
      });
    ...
    return (
      <Paper>
        <Chart data={data}>
          ...
          <BarSeries name="China" valueField="China" argumentField="year" />
          <Stack />
          <Legend
            position="bottom"
            rootComponent={Root}
